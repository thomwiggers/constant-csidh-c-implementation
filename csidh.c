...
void action(public_key *out, public_key const *in, private_key const *priv,
  ...
  // k[0] =
  // 4*347*337*331*317*311*307*293*283*277*271*269*263*251*241*239*233*227*223*211*199*
  ...
  // k[1] =
  // 4*349*337*331*317*313*307*293*283*281*271*269*263*257*241*239*233*229*223*211*199*
  ...
  // k[2] =
  // 4*349*347*331*317*313*311*293*283*281*277*269*263*257*251*239*233*229*227*211*199*
  ...
  // k[3] =
  // 4*349*347*337*317*313*311*307*283*281*277*271*263*257*251*241*233*229*227*223*199*
  ...
  // k[4] =
  // 4*349*347*337*331*313*311*307*293*281*277*271*269*257*251*241*239*229*227*223*211*
  ...
  u512 k[5] = {
      {.c = {0xf8b02e1700e5ee44, 0xf113c71293a701b4, 0x5fd885e12f150737,
             0x16bee0bbd716609d, 0xbd430c7bfb2b427a, 0xfe73b882c492385b,
             0x33aac5, 0x0}},
      {.c = {0xcc61abcfc937ab44, 0x68017a4fbca98e6, 0xb12a83d63b6d445e,
             0x529b3cbc63a7b0d, 0x17202f85bd5805a, 0xc016cb54ebcba550, 0xa3b3ad,
             0x0}},
      {.c = {0x577ac7ea0ad4df54, 0xf7fe384bdffcd347, 0x3f8f42993c859a18,
             0x80941dcb4ab8587b, 0x4f161b75c99c7f42, 0x67ec629ef79ae535,
             0x1a7b5b5, 0x0}},
      {.c = {0x3e6be99cc0eb105c, 0x93274e02ae2375f, 0xe7ae846eaa92dbd7,
             0xd2bb6f974f14003e, 0x2d60e4c0479db07f, 0xde09242d07f74906,
             0x5251c74, 0x0}},
      {.c = {0x8aa000e50dd9c3ac, 0xbc4279114b16bb9c, 0x3e8c74248a72149f,
             0x06a02aa8ba62b16c, 0x99fe9a8a6931bc52, 0x445697c5f857e177,
             0x00000000173650e4, 0x0000000000000000}}};

  u512 p_order = {.c = {0x24403b2c196b9323, 0x8a8759a31723c208,
                        0xb4a93a543937992b, 0xcdd1f791dc7eb773,
                        0xff470bd36fd7823b, 0xfbcf1fc39d553409,
                        0x9478a78dd697be5c, 0x0ed9b5fb0f251816}};
  ...
  while (isog_counter < num_isogenies) {
    ...
    if (count == my * num_batches) { // merge the batches after my rounds
      m = 0;
      last_iso[0] =
          73; // doesn't skip point evaluations anymore after merging batches
    ...
    for (uint8_t i = m; i < num_primes; i = i + num_batches) {
      if (finished[i] == true) { // depends only on randomness
        ...
        ec = lookup(i, e); // check in constant-time if normal or dummy isogeny
                           // must be computed
        bc = isequal(ec, 0);
...
bool csidh(public_key *out, public_key const *in, private_key const *priv,
           uint8_t const num_batches, int8_t const *max_exponent,
           unsigned int const num_isogenies, uint8_t const my) {
  if (!validate(in)) {
    fp_random(&out->A);
    return false;
