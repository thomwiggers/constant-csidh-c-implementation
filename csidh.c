...
const unsigned primes[num_primes] = { 349, 347, 337, 331, 317, 313, 311, 307, 293, 283, 281, 277, 271, 269, 263, 257,
		251, 241, 239, 233, 229, 227, 223, 211, 199, 197, 193, 191, 181, 179, 173, 167, 163, 157, 151, 149, 139, 137,
		131, 127, 113, 109, 107, 103, 101, 97, 89, 83, 79, 73, 71, 67, 61, 59, 53, 47, 43, 41, 37, 31, 29, 23, 19, 17,
		13, 11, 7, 5, 3, 587, 373, 367, 359, 353 };
...
inline void elligator(fp * x, const fp *A, bool sign, uint8_t index) {
...
void action(public_key *out, public_key const *in, private_key const *priv, uint8_t num_batches) {
	...
	u512 k[5] = { { .c = { 0xf8b02e1700e5ee44, 0xf113c71293a701b4, 0x5fd885e12f150737, 0x16bee0bbd716609d,
			0xbd430c7bfb2b427a, 0xfe73b882c492385b, 0x33aac5, 0x0 } }, { .c = { 0xcc61abcfc937ab44, 0x68017a4fbca98e6,
			0xb12a83d63b6d445e, 0x529b3cbc63a7b0d, 0x17202f85bd5805a, 0xc016cb54ebcba550, 0xa3b3ad, 0x0 } }, { .c = {
			0x577ac7ea0ad4df54, 0xf7fe384bdffcd347, 0x3f8f42993c859a18, 0x80941dcb4ab8587b, 0x4f161b75c99c7f42,
			0x67ec629ef79ae535, 0x1a7b5b5, 0x0 } }, { .c = { 0x3e6be99cc0eb105c, 0x93274e02ae2375f, 0xe7ae846eaa92dbd7,
			0xd2bb6f974f14003e, 0x2d60e4c0479db07f, 0xde09242d07f74906, 0x5251c74, 0x0 } }, { .c = { 0x8aa000e50dd9c3ac,
			0xbc4279114b16bb9c, 0x3e8c74248a72149f, 0x06a02aa8ba62b16c, 0x99fe9a8a6931bc52, 0x445697c5f857e177,
			0x00000000173650e4, 0x0000000000000000 } } };
	//{ .c ={0x8aa000e50dd9c3ac, 0xbc4279114b16bb9c, 0x3e8c74248a72149f, 0x6a02aa8ba62b16c, 0x99fe9a8a6931bc52, 0x445697c5f857e177,0x173650e4, 0x0} }};

	u512 p_order = { .c = { 0x24403b2c196b9323, 0x8a8759a31723c208, 0xb4a93a543937992b, 0xcdd1f791dc7eb773,
			0xff470bd36fd7823b, 0xfbcf1fc39d553409, 0x9478a78dd697be5c, 0x0ed9b5fb0f251816 } };
	...
	int32_t e[num_primes] = { 0 };
	uint32_t isog_counter = 0;
	...
	uint32_t numIsogenies = 0;
	for (int i = 0; i < num_primes; i++) {
		numIsogenies = numIsogenies + e[i];
		if (e[i] == 0) {
			u512_mul3_64(&k[i%num_batches], &k[i%num_batches], primes[i]);
		}
	}

	while (isog_counter < numIsogenies) {
		m = (m + 1) % num_batches;
		...
		if (memcmp(&A.x, &fp_0, sizeof(fp))) {
			...
			elligator_index = (elligator_index + 1) % 19;
			P.z = fp_1;
		} else {
			fp_enc(&P.x, &p_order);
			P.z = fp_1;
...
//		return false;
//	}
	action(out, in, priv, num_batches);
